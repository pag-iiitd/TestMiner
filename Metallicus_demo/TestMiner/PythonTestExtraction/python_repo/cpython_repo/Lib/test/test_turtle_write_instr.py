...
Vec2D = turtle.Vec2D
test_config = "width = 0.75\nheight = 0.8\ncanvwidth = 500\ncanvheight = 200\nleftright = 100\ntopbottom = 100\nmode = world\ncolormode = 255\ndelay = 100\nundobuffersize = 10000\nshape = circle\npencolor  = red\nfillcolor  = blue\nresizemode  = auto\nvisible  = None\nlanguage = english\nexampleturtle = turtle\nexamplescreen = screen\ntitle = Python Turtle Graphics\nusing_IDLE = ''\n"
test_config_two = '# Comments!\n# Testing comments!\npencolor  = red\nfillcolor  = blue\nvisible  = False\nlanguage = english\n# Some more\n# comments\nusing_IDLE = False\n'
invalid_test_config = '\npencolor = red\nfillcolor: blue\nvisible = False\n'
...
class TurtleConfigTest(unittest.TestCase):
    ...
    def test_config_dict(self):
        ...
        print([str(test_config), 'test_config', ('python_' + str(type(test_config)).split("'")[1])], ', ', sep='', end='')
        parsed_cfg = turtle.config_dict(cfg_name)
        print([str(cfg_name), 'cfg_name', ('python_' + str(type(cfg_name)).split("'")[1])], ', ', sep='', end='')
        expected = {
            'width': 0.75,
            'height': 0.8,
            'canvwidth': 500,
        ...
        print([str(expected), 'EXP_op', ('python_' + str(type(expected)).split("'")[1])], '], [', sep='', end='')

    def test_partial_config_dict_with_commments(self):
        cfg_name = self.get_cfg_file(test_config_two)
        print([str(test_config_two), 'test_config_two', ('python_' + str(type(test_config_two)).split("'")[1])], ', ', sep='', end='')
        parsed_cfg = turtle.config_dict(cfg_name)
        print([str(cfg_name), 'cfg_name', ('python_' + str(type(cfg_name)).split("'")[1])], ', ', sep='', end='')
        expected = {
            'pencolor': 'red',
    ...
    def test_config_dict_invalid(self):
        ...
        print([str(invalid_test_config), 'invalid_test_config', ('python_' + str(type(invalid_test_config)).split("'")[1])], ', ', sep='', end='')
