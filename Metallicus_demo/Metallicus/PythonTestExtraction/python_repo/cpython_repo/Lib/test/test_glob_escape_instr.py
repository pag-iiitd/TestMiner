...
import sys
import unittest
from test.support import TESTFN, skip_unless_symlink, can_symlink, create_empty_file, change_cwd
...
class GlobTests(unittest.TestCase):
    ...
    def check_escape(self, arg, expected):
        ...
        print([str(arg), 'pathname', ('python_' + str(type(arg)).split("'")[1])], ', ', [str(expected), 'EXP_op', ('python_' + str(type(expected)).split("'")[1])], '], [', sep='', end='')
        self.assertEqual(glob.escape(os.fsencode(arg)), os.fsencode(expected))
        print([str(os.fsencode(arg)), 'pathname', ('python_' + str(type(os.fsencode(arg))).split("'")[1])], ', ', [str(os.fsencode(expected)), 'EXP_op', ('python_' + str(type(os.fsencode(expected))).split("'")[1])], '], [', sep='', end='')
    ...
    def test_recursive_glob(self):
        eq = self.assertSequencesEqual_noorder
        full = [('EF',), ('ZZZ',), ('a',), ('a', 'D'), ('a', 'bcd'), ('a', 'bcd', 'EF'), ('a', 'bcd', 'efg'), ('a', 'bcd', 'efg', 'ha'), ('aaa',), ('aaa', 'zzzF'), ('aab',), ('aab', 'F')]
        if can_symlink():
        ...
        eq(self.rglob('a', '**'), self.joins(('a', ''), ('a', 'D'), ('a', 'bcd'), ('a', 'bcd', 'EF'), ('a', 'bcd', 'efg'), ('a', 'bcd', 'efg', 'ha')))
        ...
        with change_cwd(self.tempdir):
            ...
            eq(glob.glob(join(os.curdir, '**'), recursive=True), ([join(os.curdir, '')] + [join(os.curdir, *i) for i in full]))
            eq(glob.glob(join(os.curdir, '**', ''), recursive=True), ([join(os.curdir, '')] + [join(os.curdir, *i) for i in dirs]))
