...
from distutils.util import get_platform, convert_path, change_root, check_environ, split_quoted, strtobool, rfc822_escape, byte_compile, grok_environment_error
...
class UtilTestCase(support.EnvironGuard, unittest.TestCase):
    ...
    def test_split_quoted(self):
        ...
        print([str('""one"" "two" \'three\' \\four'), 's', ('python_' + str(type('""one"" "two" \'three\' \\four')).split("'")[1])], ', ', [str(['one', 'two', 'three', 'four']), 'EXP_op', ('python_' + str(type(['one', 'two', 'three', 'four'])).split("'")[1])], '], [', sep='')
