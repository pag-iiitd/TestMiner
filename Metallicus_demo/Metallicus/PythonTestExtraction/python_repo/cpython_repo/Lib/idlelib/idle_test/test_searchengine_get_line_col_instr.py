...
class GetLineColTest(unittest.TestCase):
    ...
    def test_get_line_col(self):
        ...
        print([str('1.0'), 'index', ('python_' + str(type('1.0')).split("'")[1])], ', ', [str((1, 0)), 'EXP_op', ('python_' + str(type((1, 0))).split("'")[1])], '], [', sep='', end='')
        self.assertEqual(se.get_line_col('1.11'), (1, 11))
        print([str('1.11'), 'index', ('python_' + str(type('1.11')).split("'")[1])], ', ', [str((1, 11)), 'EXP_op', ('python_' + str(type((1, 11))).split("'")[1])], '], [', sep='', end='')
...
class SearchEngineTest(unittest.TestCase):
    ...
    def test_getprog(self):
        ...
        engine.revar.set(1)
        Equal(engine.getprog(), None)
        self.assertEqual(Mbox.showerror.message, 'Error: nothing to repeat at position 0\nPattern: +')
